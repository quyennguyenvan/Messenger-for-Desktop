 ...
 */
class Win32NativeNotifier extends _base.default {
  ...
  fireNotification({
    ...
    const display = _electron.screen.getDisplayNearestPoint({
      x: currentWindowPos[0],
      y: currentWindowPos[1]
    });
    let timerId;
    this.notifications[identifier] = new _electron.BrowserWindow({
      width: 300,
      height: 80,
      title,
      frame: false,
    ...
    const htmlUrl = 'file://' + _path.default.join(_electron.app.getAppPath(), 'html', 'shim-notification.html') + '?' + ['identifier=' + encodeURIComponent(identifier), 'title=' + encodeURIComponent(title || global.manifest.productName + ' notification'), 'body=' + encodeURIComponent(body || 'New message.'), 'icon=' + encodeURIComponent(icon || '../images/default_avatar.png'), 'footer=' + encodeURIComponent(footer || global.manifest.productName), 'timeout=' + (timeout || 5000)].join('&');
...
var _default = exports.default = Win32NativeNotifier;